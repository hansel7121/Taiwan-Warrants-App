...
# ── warrants never enter the short side ──────────────────────────────────────
#
# The LP's long basket may hold warrants and options; its short basket holds
# options ONLY. Selling a warrant is shorting an American claim the holder may
# exercise before T*, which the one-period payoff model cannot bound, so that
# direction lives in Direct Match and not here. These pin the exclusion.

def test_warrants_never_become_short_legs():
    """A warrant with a live bid AT the horizon is still long-side only."""
    w = [warrant("W1", "Call", 100.0, 30, ask=12.5)]      # helper gives bid = ask*0.98
    longs, shorts, _ = static_arb._build_legs(
        pd.DataFrame(w), pd.DataFrame(), 30, M, R)
    assert [l["code"] for l in longs] == ["W1"]
    assert shorts == []


def test_buy_option_sell_warrant_direction_is_unreachable():
    """arb_logic's negative direction: warrant bid far above the option ask at the
    same strike and expiry. It is real, and the LP must still not emit it — the
    variable set has no sell-warrant column."""
    w = [warrant("W2", "Call", 100.0, 30, ask=12.5, ask_qty=10)]
    o = [option("C100", "Call", 100.0, 30, bid=1.9, ask=2.0)]
    assert solve(w, o, 30)[0] is None


def test_warrant_bid_depth_is_never_counted_as_dropped():
    """No size at the warrant bid is not a dropped leg — the sell side does not
    exist, so there is nothing to drop. Only the missing ASK size counts."""
    w = [warrant("W1", "Call", 100.0, 30, ask=12.5, ask_qty=10)]
    w[0]["bid_qty"] = 0
...
def test_horizons_come_from_option_expiries_only():
    """A warrant expiry that no option shares is not a horizon: nothing can settle
    there, because only short legs settle at T* and shorts are options."""
    w = [warrant("W-45", "Call", 100.0, 45, ask=10.0)]
    o = [option("C100", "Call", 100.0, 30, bid=1.9, ask=2.0)]
    _, shorts, _ = static_arb._build_legs(
        pd.DataFrame(w), pd.DataFrame(o), 45, M, R)
    assert shorts == []
