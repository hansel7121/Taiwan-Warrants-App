...
KGI_FIELDS = {"person_id": "A123456789", "person_pwd": "hunter2-not-in-any-log"}
FUBON_FIELDS = {"person_id": "B987654321", "password": "fubon-pw-not-in-any-log",
...
def test_upsert_persists_only_ciphertext(store):
    ...
    assert KGI_FIELDS["person_pwd"] not in str(row)
    assert KGI_FIELDS["person_id"] not in str(row)
...
def test_get_credential_round_trips(store):
    ...
    assert got["person_id"] == KGI_FIELDS["person_id"]
    assert got["person_pwd"] == KGI_FIELDS["person_pwd"]
...
def test_list_credentials_never_exposes_secrets(store, capsys):
    ...
    assert KGI_FIELDS["person_pwd"] not in str(listed)
    assert KGI_FIELDS["person_pwd"] not in capsys.readouterr().out


def test_list_credentials_is_scoped_to_the_user(store):
    credentials.upsert_credential(USER, "kgi", KGI_FIELDS)
    credentials.upsert_credential("other-user", "fubon", FUBON_FIELDS)
...
def test_remove_credential_deletes_only_that_broker(store):
    credentials.upsert_credential(USER, "kgi", KGI_FIELDS)
    credentials.upsert_credential(USER, "fubon", FUBON_FIELDS)
...
def test_cert_upload_sets_path_and_downloads_back(store):
    # Fubon is the broker that actually carries a cert (its .pfx). KGI's cert is
    # a manual one-time CLI step done outside the app (docs/adr/0004).
    credentials.upsert_credential(USER, "fubon", FUBON_FIELDS)

    credentials.upload_cert(USER, "fubon", b"cert-bytes", "pfx")

    assert store.buckets == ["broker-certs"]
    expected = f"{USER}/fubon/cert.pfx"
    ...
    assert credentials.download_cert(USER, "fubon") == b"cert-bytes"
