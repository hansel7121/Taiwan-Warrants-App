...
// ── Broker worker status ─────────────────────────────────────────────
// /broker/status returns two lists: what the worker reports (status) and what
// the user asked for (desired). They are shown together because the gap
// between them is the interesting part — "connect requested, nothing
// reported" is a worker that has not picked the request up yet.
const WORKER_BADGE = {
  connected:    ["Connected", "mc-open"],
  reconnecting: ["Reconnecting", "mc-after"],
  stopped:      ["Stopped", "mc-closed"],
  disconnected: ["Disconnected", "mc-bad"],
};

function _workerRow(broker, status, want) {
  const [txt, cls] = WORKER_BADGE[status] || ["No worker", "mc-closed"];
  const wantTag = want ? '<span class="mc-want">want ' + want + "</span>" : "";
  return '<div class="mc-row"><span class="mc-lbl">' + broker.toUpperCase() +
    wantTag + '</span><span class="mc-badge ' + cls + '">' + txt + "</span></div>";
}

async function refreshWorkerStatus() {
  const el = document.getElementById("worker-status-rows");
  if (!el) return;
  let body;
  try {
    ...
    el.innerHTML = '<div class="mc-row"><span class="mc-lbl">status</span>' +
      '<span class="mc-badge mc-closed">Unavailable</span></div>';
    return;
  }
  const byBroker = {};
  (body.desired || []).forEach(r => { byBroker[r.broker] = { want: r.desired_state }; });
  (body.status || []).forEach(r => {
    byBroker[r.broker] = Object.assign(byBroker[r.broker] || {}, { status: r.status });
  });
  const brokers = Object.keys(byBroker).sort();
  // Nothing on either side means no broker account has ever been asked to
  // connect — a different thing from a worker reporting "disconnected".
  el.innerHTML = brokers.length
    ? brokers.map(b => _workerRow(b, byBroker[b].status, byBroker[b].want)).join("")
    : '<div class="mc-row"><span class="mc-lbl">none</span>' +
      '<span class="mc-badge mc-closed">Not requested</span></div>';
}
