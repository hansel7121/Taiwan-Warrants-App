...
- **The static-arb LP** (`logic/static_arb.py`). `linprog(method="highs")` returns *a* vertex of the optimal face; these LPs are structurally degenerate, so the optimal value is unique but `res.x` is not. A different vertex flows through the integer-repair step (`_round_longs`/`_round_shorts` in `_solve_horizon`) into a different `legs` list, which is the Static Arb tab's entire visible output. Not a tolerance problem — the answer is legitimately non-unique. **Revised by ADR-0005**: the non-uniqueness is real and was measured at ~1% of horizons, which turned out to be a price worth paying for 500 ms → 7.4 ms once tick replay made the LP the binding constraint.
