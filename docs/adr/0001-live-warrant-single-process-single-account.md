...
**Considered:** a 3-container design (`web-api` + `broker-connector` + `redis`) backed by a pool of 5 Fubon accounts (35 connections / 10,500 subscriptions), built up front for eventual multi-user scale. Rejected for v1 as premature — it solves a concurrency problem (many `web-api` replicas needing one shared broker feed) that doesn't exist yet, at the cost of Redis, SSE, and cross-process failure handling.
