...
Two channels fire independently and redundantly on purpose: a dropped feed
during trading hours is exactly the event where Slack being down, or a phone not
receiving push, must not mean no alert at all.
...
WHEN to alert is not decided here. Nothing calls this yet — the reconnect
supervisor that owns the grace threshold and the once-per-incident gate arrives
with the live-tick pipeline — but the channel plumbing lands with the worker so
the first real outage has somewhere to report to.
"""
import logging
import os
