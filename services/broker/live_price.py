...
{"price": float, "ts": datetime, "broker": str, "qty": int | None, "instrument": str}
...
market hours — see module docstring in live_depth.py / issue #56: a poller
sharing one client/connection pool with user-facing requests let a poller
hiccup stall an unrelated login for up to 2 minutes.
...
def entry(code):
    """(epoch_float, {"price", "ts", "broker", "qty", "instrument"}) for `code`, or None if never seen."""
    return _cache.entry(code)


def snapshot(codes):
    """{code: {"price", "ts", "broker", "qty", "instrument"}} for those `codes` that have a tick."""
    out = {}
    for code in codes:
    ...
    for row in rows:
        ...
        _cache.set(
            ...
            {"price": row["price"], "ts": ts, "broker": row["broker"],
             "qty": row.get("qty"), "instrument": row.get("instrument", "warrant")},
            ts=ts.timestamp(),
        )
    return len(rows)
