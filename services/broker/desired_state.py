...
def list_all_desired_states():
    ...
    The worker serves all users from one process (docs/adr/0001: every user's
    Broker Account feeds one shared pool), so unlike the per-user reads above it
    has no caller-scoped user_id to filter on.
    """
    r = db._run(
        lambda c: c.table(DESIRED_TABLE)
