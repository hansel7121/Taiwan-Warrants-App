-- Migration 016: broker_desired_state + worker_status.
...
--
-- Ported from live-arb's migration 006 (same table shapes, unchanged) — these
-- two tables are load-bearing for this branch's own code
-- (services/broker/desired_state.py, broker_worker.py, app.py's
-- /broker/connect|disconnect|status routes) but were never carried forward
-- into this branch's own migrations/schema.sql. Renumbered to 016 since 007
-- (arb_signals) stayed out of scope with the rest of the #19 epic.
--
