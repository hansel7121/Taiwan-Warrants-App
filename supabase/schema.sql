...
-- ─────────────────────────────────────────────────────────────────────────────
-- Broker desired-state + worker status (migration 016).
--
-- The UI and the Singapore worker are separate processes that never call each
-- other — the browser cannot reach the worker's box, and the worker is
-- offline whenever the user clicks. So the two halves communicate through
-- these two tables instead: the user writes what they *want*
-- (broker_desired_state), the worker writes what *is* (worker_status), and
-- each side polls the other.
--
-- broker_desired_state is deliberately not a queue: the worker only ever
-- needs the newest intent, and a click-happy user toggling connect/disconnect
-- five times should not make the worker replay five transitions. One row per
...
-- like the md_* tables) because a user's own client must not be able to forge
-- a "connected" row — the status is only meaningful if the worker is its
-- sole author. The browser reads it through /broker/status, which is
-- server-side and scopes the query to the caller.
